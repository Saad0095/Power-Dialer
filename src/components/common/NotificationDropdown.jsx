...
export default function NotificationDropdown() {
  ...
  const handleNotificationClick = async (notification) => {
    ...
    else if (notification.type === 'follow_up' || notification.type === 'appointment') {
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((notification) => (
                <div
                  ...
                  className={`p-4 border-b border-slate-100 dark:border-slate-700/50 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/30 transition ${!notification.isRead ? 'bg-blue-50/50 dark:bg-blue-500/5' : ''
                    }`}
                >
                  <div className="flex justify-between items-start mb-1">
                    <h4 className={`text-sm ${!notification.isRead ? 'font-semibold text-slate-900 dark:text-white' : 'font-medium text-slate-700 dark:text-slate-300'}`}>
                      {notification.title}
                    </h4>
                    {!notification.isRead && (
