...
import { Users, PhoneCall, RefreshCw, Calendar, Clock3 } from "lucide-react";
    ...
    <div className="bg-linear-to-br from-slate-100 to-slate-50 dark:from-slate-800 dark:to-slate-700 rounded-lg shadow-xl dark:shadow-slate-900/30 p-6 border border-slate-200 dark:border-slate-700 space-y-4">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">
            {selectedDate
              ? `Agent Calls (${selectedDate})`
              : "Agent Calls (Past 12 Hours)"}
          </h2>
        ...
        <div className="flex items-end gap-2">
          ...
          >
            <RefreshCw
              className={`w-4 h-4 ${isDailyCallsLoading ? "animate-spin" : ""}`}
            />
            Refresh
          </button>
        </div>
      ...
      {dailyCallData.windowStart && dailyCallData.windowEnd && (
        <p className="font-bold text-white-600 dark:text-yellow-600">
          Window: {new Date(dailyCallData.windowStart).toLocaleString()} -{" "}
          {new Date(dailyCallData.windowEnd).toLocaleString()}
        </p>
      )}
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
        <div className="bg-slate-100 dark:bg-slate-900/40 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-2xl font-bold text-cyan-700 dark:text-cyan-400">
            {dailyCallData.summary.totalCalls}
          </p>
        </div>
        <div className="bg-slate-100 dark:bg-slate-900/40 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
          <p className="text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">
        ...
        <div className="bg-slate-100 dark:bg-slate-900/40 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-2xl font-bold text-purple-700 dark:text-purple-400 flex items-center gap-2">
        ...
        <div className="bg-slate-100 dark:bg-slate-900/40 rounded-lg p-3 border border-slate-200 dark:border-slate-700">
          ...
          <p className="text-2xl font-bold text-gray-700 dark:text-gray-400">
            {dailyCallData.summary.totalAgents}
          </p>
        </div>
      </div>
      {dailyCallsError && (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-200/70 dark:bg-slate-900/50">
              <tr>
                ...
                <th className="text-right px-4 py-3 text-slate-700 dark:text-slate-300 font-semibold">
                  Calls
                </th>
                <th className="text-right px-4 py-3 text-slate-700 dark:text-slate-300 font-semibold">
                  Followups
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {dailyCallData.agents.length === 0 ? (
                <tr>
                  <td
                    colSpan={5}
                    className="px-4 py-6 text-center text-slate-600 dark:text-slate-400"
                  >
                    {isDailyCallsLoading
                      ? "Loading daily call data..."
                      : "No agents found"}
                  </td>
                </tr>
              ) : (
                dailyCallData.agents.map((agent) => (
                  <tr
                    key={agent.agentId}
                    className="bg-white dark:bg-slate-900/20"
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300 font-semibold">
                    ...
                    <td className="px-4 py-3 text-right">
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300 font-semibold">
